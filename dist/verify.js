...
export async function verifyPluginBoot(spec) {
    ...
    let version = 'unknown';
    try {
        version = await dshVersion(dshPath);
    }
    catch { /* unknown */ }
    let temp;
    try {
    ...
    try {
        ...
        const verdictState = probeStateToVerdict[probe.state] ?? 'probe-inert';
        const v = {
...
function diagnosticFromOutcome(outcome, diagnostics, _candidateDir, _services, _dshPath) {
    const stderrTrim = outcome.stderr.trim();
    if (stderrTrim !== '')
        diagnostics.push(`stderr: ${stderrTrim.slice(0, 1600)}`);
    if (outcome.exitCode !== null)
        diagnostics.push(`exit code: ${outcome.exitCode}`);
}
function probeDiag(state) {
    switch (state) {
...
export async function canaryState(spec) {
