 ...
 */
export function parseCandidate(spec: string): CandidateRef {
  const trimmed = spec.trim()
  if (trimmed === '') throw new Error('dsh-canary: plugin spec is empty')
  ...
  if (at > 0 && !trimmed.startsWith('@')) {
    ...
    if (PACKAGE_NAME_RE.test(name)) return { name, spec: trimmed, local: false }
  }

  // Bare name (possibly scoped).
  if (PACKAGE_NAME_RE.test(trimmed)) return { name: trimmed, spec: trimmed, local: false }
...
function nameOfDirectory(p: string): string | null {
  let dir = p
  try {
    if (statSync(p).isFile()) dir = dirnameOf(p)
  } catch { /* fall through */ }
  try {
 ...
 */
export function candidateServices(packageDir: string): string[] {
  const rows = readBundlePatchRows(packageDir)
  const list: string[] = []
  for (const name of rows.names) if (!list.includes(name)) list.push(name)
