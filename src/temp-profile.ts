...
export function probePackageDir(): string {
 ...
 */
export function buildTempProfile(
  sourceProfileDir: string,
  candidate: CandidateRef,
  home: string,
  opts: { keep?: boolean; probe?: boolean } = {},
): TempProfile {
  ...
  if (existsSync(srcNm)) mirrorNodeModules(srcNm, nm)
...
function candidateServicesOf(candidateDir: string): string[] {
  try {
    const rows = readBundlePatchRows(candidateDir)
    const list: string[] = []
    for (const n of rows.names) if (!list.includes(n)) list.push(n)
    for (const id of rows.insertedIds) if (!list.includes(id)) list.push(id)
    return list
  } catch {
    return []
  }
 ...
 */
function ensureCandidateInProfile(sourceProfileDir: string, candidate: CandidateRef, nm: string): string {
  const candidateDir = join(nm, candidate.name)
  if (existsSync(join(candidateDir, 'package.json'))) return candidateDir
  ...
  if (candidate.local && localPath !== null && existsSync(join(localPath, 'package.json'))) {
    ...
    try { symlinkSync(localPath, candidateDir) } catch {
  ...
  if (existsSync(join(sourceProfileDir, 'node_modules', candidate.name, 'package.json'))) {
    ...
    ensureParent(candidateDir)
    try { symlinkSync(realpathSync(srcDir), candidateDir) } catch {
  ...
  throw new Error(
    `dsh-canary: candidate ${JSON.stringify(candidate.name)} (spec ${JSON.stringify(candidate.spec)}) ` +
...
function mirrorNodeModules(src: string, dest: string): void {
  let entries: string[]
  try { entries = readdirSync(src, { withFileTypes: true }).filter((d) => d.name !== '.package-lock.json' && d.name !== '.modules.yaml' && d.name !== '.cache').map((d) => d.name) } catch { return }
  for (const entry of entries) {
    ...
    if (entry.startsWith('@')) {
      ...
      for (const pkg of pkgNames) {
        ...
        if (target !== null) safeSymlink(target, join(destPath, pkg))
      }
      continue
    }
    const target = realpathOrNull(srcPath)
    if (target !== null) safeSymlink(target, destPath)
  }
}
...
function safeSymlink(target: string, dest: string): void {
  try { rmSync(dest, { recursive: true, force: true }) } catch { /* ignore */ }
  try { symlinkSync(target, dest) } catch { /* ignore */ }
}
...
function copyIfExists(src: string, dest: string): void {
  ...
  try { cpSync(src, dest, { recursive: true, force: true, dereference: false }) } catch { /* best-effort */ }
}
 ...
 */
function writeProbeOverlay(dir: string, candidate: string, services: string[]): string {
  ...
  if (services.length > 0) {
    ...
    for (const s of services) lines.push(`          - '${yamlQuote(s)}'`)
  }
  lines.push('')
