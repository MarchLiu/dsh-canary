...
export function webserverBindPatch(host: string, port: number): string {
  return [
 ...
 */
export function planDebugWeb(spec: CanarySpec, dir: string): DebugWebPlan {
  const host = '0.0.0.0'
  const port = 3080
...
export function debugWebVerdict(spec: CanarySpec, plan: DebugWebPlan): Verdict {
  return {
    ok: true,
    mode: 'debug',
