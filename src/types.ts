...
export type VerdictState =
  | 'boot-ok'       // boot reached AppReady / the ready marker, no fail-loud
  | 'probe-pass'    // boot-ok AND the bounded functional probe resolved the plugin's primary service
  | 'probe-fail'    // boot-ok AND the probe explicitly observed a broken primary service
  | 'probe-timeout' // boot-ok AND the probe could not decide before its deadline (non-blocking)
  | 'probe-inert'   // boot-ok AND no meaningful service was declarable (non-blocking)
  | 'boot-fail'     // the boot itself failed: crash / fail-loud / hang
